        ...
        for (const auto& primitive : primitives)
        {
            if (ImGui::Selectable(primitive.Label))
            {
                ...
                case OBJ_FBX:
                    ...
                    break;
                case OBJ_SHARKRY:
                    SpawnedActor = World->SpawnActor<ARandCharacter>();
                    SpawnedActor->SetActorLabel(TEXT("OBJ_SHARKRY"));
                    SpawnedActor->SetActorScale(FVector(.01f, .01f, .01f));                    SpawnedActor->SetActorScale(FVector(.01f, .01f, .01f));
                case OBJ_CAMERA:
                case OBJ_PLAYER:
                case OBJ_END:
                    break;
...
void ControlEditorPanel::CreatePIEButton(const ImVec2 ButtonSize, ImFont* IconFont) const
{
    ...
    const float CenterX = (WindowSizeX - ButtonSize.x) / 2.f;
